...
def log_loss(
    labels: np.ndarray, pred: np.ndarray, from_logits=False, small_value=1e-8
) -> np.ndarray:
    ...
    if labels.shape[0] != pred.shape[0]:
        raise ValueError(
            "labels and pred should have the same number of examples,",
    ...
    if pred.size == pred.shape[0]:
        ...
        if classes.min() < 0 or classes.max() > 1:
            raise ValueError(
                "Each value in pred is a scalar, so labels are expected",
...
def multilabel_bce_loss(
    ...
    if not from_logits and ((pred < 0.0) | (pred > 1.0)).any():
        raise ValueError(
            (
                "Prediction probabilities are not in [0, 1] and "
