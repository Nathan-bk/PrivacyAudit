...
from Attacks.data_structures import (
    AttackInputData,
    AttackResults,
    AttackType,
    DataSize,
...
from Attacks.dataset_slicing import get_single_slice_specs, get_slice
...
def _run_threshold_attack(attack_input: AttackInputData):
    ...
    # NOTE just retrieves the scores provided by the run_score function
    # that went through the attack_input object
    # and applies the sklearn roc_curve function to it
    ...
    if attack_input.is_multilabel_data():
        logging.info(
            (
                "For multilabel data, when a threshold attack is requested, "
...
def _run_threshold_entropy_attack(attack_input: AttackInputData):
    ...
    if attack_input.is_multilabel_data():
        raise NotImplementedError(
            ("Entropy-based attacks are not implemented for " "multilabel data.")
        )
    ntrain, ntest = attack_input.get_train_size(), attack_input.get_test_size()
    fpr, tpr, thresholds = metrics.roc_curve(
        np.concatenate((np.zeros(ntrain), np.ones(ntest))),
...
def _run_attack(
    ...
    if (
        min(attack_input.get_train_size(), attack_input.get_test_size())
        < min_num_samples
    ):
        return None
...
def _compute_membership_probability(
    attack_input: AttackInputData, num_bins: int = 15
) -> SingleMembershipProbabilityResult:
    ...
    if attack_input.loss_train is not None and attack_input.loss_test is not None:
        ...
        attack_input.entropy_train is not None and attack_input.entropy_test is not None
    ):
...
# TODO : Delete this function which is not used


def run_membership_probability_analysis(
    attack_input: AttackInputData, slicing_spec: SlicingSpec = None
) -> MembershipProbabilityResults:
...
def _compute_missing_privacy_report_metadata(
    metadata: PrivacyReportMetadata, attack_input: AttackInputData
) -> PrivacyReportMetadata:
    ...
    if attack_input.is_multilabel_data():
        ...
        if attack_input.probs_train is None and attack_input.logits_train is not None:
            logits_or_probs_train = sigmoid_func(attack_input.logits_train)
        else:
            logits_or_probs_train = attack_input.probs_train
        if attack_input.probs_test is None and attack_input.logits_test is not None:
    ...
    if metadata.accuracy_train is None:
        metadata.accuracy_train = accuracy_fn(
            logits_or_probs_train, attack_input.labels_train
        )
    if metadata.accuracy_test is None:
        metadata.accuracy_test = accuracy_fn(
            logits_or_probs_test, attack_input.labels_test
        )
    loss_train = attack_input.get_loss_train()
    loss_test = attack_input.get_loss_test()
